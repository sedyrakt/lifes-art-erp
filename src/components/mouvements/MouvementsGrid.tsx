// src/components/mouvements/MouvementsGrid.tsx
...
const MouvementsGrid: React.FC<MouvementsGridProps> = ({ mouvements, getTypeColor, getTypeIcon, getTypeLabel, onView, onDelete, onBulkDelete, imageUrls = {}, loadImageForMouvement, isDark: isDarkProp }) => {
  const { isDark: themeIsDark } = useTheme(); const isDark = isDarkProp !== undefined ? isDarkProp : themeIsDark;

  React.useEffect(() => {
    if (!loadImageForMouvement) return;
    mouvements.forEach((mouvement) => { if (imageUrls[mouvement.id] === undefined) loadImageForMouvement(mouvement); });
  }, [mouvements, imageUrls, loadImageForMouvement]);

  if (mouvements.length === 0) {
    return (<div className={`flex min-h-[280px] flex-col items-center justify-center rounded-xl border bg-white px-6 py-12 text-center shadow-sm dark:bg-[#111c30] dark:shadow-[0_12px_40px_rgba(0,0,0,0.18)] ${isDark ? 'border-white/[0.14]' : 'border-slate-300'}`}>
      <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-xl bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400"><Activity size={24} strokeWidth={1.8} /></div>
      <h3 className="text-[16px] font-semibold text-slate-900 dark:text-slate-100">Aucun mouvement</h3>
      <p className="mt-1 max-w-sm text-[14px] leading-relaxed text-slate-500 dark:text-slate-400">Aucun mouvement de stock n'a été enregistré pour le moment.</p>
    </div>);
  }
  ...
  return (<div className={`grid grid-cols-1 gap-4 p-4 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4 ${isDark ? 'bg-[#111c30]' : 'bg-white'}`}>
    {mouvements.filter(Boolean).map((mouvement) => {
      ...
      const productName = mouvement.produit_nom?.trim() || 'Produit inconnu'; const productCode = mouvement.produit_code?.trim() || 'Sans code';
      const reference = mouvement.reference?.trim() || '-'; const dateDisplay = formatDate(mouvement.date_mouvement); const timeDisplay = formatTime(mouvement.date_mouvement);

      let quantityConfig;
      if (isEntree) { quantityConfig = { prefix: '+', wrapper: 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400', icon: <ArrowDown size={12} /> }; }
