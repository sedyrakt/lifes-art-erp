// ============================================================
// src/components/sorties/SortiesPagination.tsx - PREMIUM KEYSET (UNIFIED)
// ⭐ Design mitovy tanteraka amin'ny EntreesPagination
// ============================================================
import React from 'react';
import { ChevronLeft, ChevronRight } from 'lucide-react';
import { useTheme } from '../../contexts/ThemeContext';

interface SortiesPaginationProps {
  currentPage: number;
  totalItems: number;
  hasMore: boolean;
  onNext: () => void;
...
const SortiesPagination: React.FC<SortiesPaginationProps> = ({ 
  ...
  // ⭐ Loko mifanaraka amin'ny Dashboard (ProduitsTable / EntreesPagination)
  ...
  return (
    <div className={`mt-6 flex w-full items-center justify-between rounded-xl border px-4 py-3 shadow-sm transition-all duration-200 ${bgClass} ${borderClass}`}>
      
      {/* ⭐ Gauche : Total (Uppercase tahaka ny an'ny Produit) */}
      <div className={`text-[13px] font-bold uppercase tracking-wider ${textMuted}`}>
        TOTAL : <span className={`font-black ${textStrong}`}>{totalItems}</span> sortie{totalItems > 1 ? 's' : ''}
      </div>
...
export default SortiesPagination;