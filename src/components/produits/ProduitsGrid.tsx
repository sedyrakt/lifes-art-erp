          ...
          >
            <div className="relative h-48 w-full overflow-hidden bg-slate-100 dark:bg-slate-800">
              {imageUrl && !hasImageError ? (
                ...
                <div className="flex h-full w-full items-center justify-center"><span className="text-4xl font-bold text-indigo-600 dark:text-indigo-400">{initial}</span></div>
              )}
              <div className="absolute right-2 top-2">{renderStatusBadge(produit.status)}</div>
            </div>

            <div className="flex flex-1 flex-col p-4">
              <div className="mb-1 flex items-start justify-between">
                <h4 className="text-[14.5px] font-semibold text-slate-900 line-clamp-2 dark:text-slate-100" title={produit.nom}>{produit.nom}</h4>
                {produit.fournisseur_nom && <span className="inline-flex items-center gap-1 text-[13px] text-slate-400 dark:text-slate-500"><Truck size={12} /><span className="truncate max-w-[80px]">{produit.fournisseur_nom}</span></span>}
              </div>
              <div className="text-[14.5px] text-slate-500 dark:text-slate-400">
                <span className="font-mono">{produit.code || '—'}</span>
                {produit.categorie_nom && <span className="ml-2 rounded-lg bg-slate-100 px-2 py-0.5 text-[13px] dark:bg-slate-800 dark:text-slate-300">{produit.categorie_nom}</span>}
              </div>
                ...
                <div className="flex flex-1 flex-col">
                  <div className="flex items-baseline justify-between text-[14.5px]">
                    <span className={`font-bold ${isRupture ? 'text-rose-600 dark:text-rose-400' : isAlert ? 'text-amber-600 dark:text-amber-400' : 'text-slate-900 dark:text-slate-100'}`}>
                      {stock} <span className="text-[12px] font-medium text-slate-400">{produit.unite || 'p.'}</span>
                    </span>
                    <span className="text-[12px] font-semibold">{isRupture ? 'Rupture' : isAlert ? 'Faible' : 'OK'}</span>
                  </div>
              ...
              <div className="mt-2 flex items-baseline justify-between">
                <div>
                  <span className="text-[16px] font-bold text-slate-900 dark:text-slate-100">{formatMoney(prixVente)}</span>
                  <span className="ml-2 text-[13px] text-slate-400 line-through dark:text-slate-500">{formatMoney(prixAchat)}</span>
                </div>
                <span className="inline-flex min-w-[34px] items-center justify-center rounded-lg border border-indigo-100 bg-indigo-50 px-2 py-1 text-[14.5px] font-bold text-indigo-700 dark:border-indigo-500/10 dark:bg-indigo-500/10 dark:text-indigo-300">{produit.nb_commandes ?? 0}</span>
              </div>
              ...
              <div className="mt-4 flex items-center gap-2 border-t border-slate-200 pt-3 dark:border-slate-700" onClick={(e) => e.stopPropagation()}>
                ...
                <button type="button" title="Supprimer" onClick={() => onDelete(produit)} className="flex-1 rounded-lg bg-rose-100 px-3 py-2 text-rose-700 transition hover:bg-rose-200 hover:text-rose-800 dark:bg-rose-900/30 dark:text-rose-400 dark:hover:bg-rose-800/50"><Trash2 size={16} className="mx-auto" /></button>
                <button type="button" title="Nouvelle commande" onClick={() => onNewCommande(produit)} className="flex-1 rounded-lg bg-indigo-600 px-3 py-2 text-white transition hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600"><ShoppingBag size={16} className="mx-auto" /></button>
              </div>
            </div>
          </div>
        );
