      ...
      <table className="w-full min-w-[760px] border-separate border-spacing-0 text-left">
        ...
        <tbody className="bg-white dark:bg-[#111c30]">
          {displayedEmployes.map((employee, index) => {
            ...
            return (<tr key={employeeId || `employee-${index}`} className="group h-[58px] cursor-pointer transition-colors duration-150 hover:bg-slate-50 dark:hover:bg-slate-800/40">
              <td className={`border px-5 py-2.5 align-middle ${cellBorderColor}`}><span className="text-[13px] text-slate-500 dark:text-slate-400">{(employePage - 1) * ITEMS_PER_PAGE + index + 1}</span></td>
              <td className={`border px-5 py-2.5 align-middle ${cellBorderColor}`}><div className="flex items-center gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full bg-indigo-500/10 text-[12px] font-bold text-indigo-600 dark:text-indigo-400">{initials}</div><div className="min-w-0"><p className="truncate text-[14px] font-medium text-slate-900 dark:text-slate-100">{prenom} {nom}</p>{poste && <p className="mt-0.5 truncate text-[13px] text-slate-500 dark:text-slate-400">{poste}</p>}</div></div></td>
              <td className={`border px-5 py-2.5 align-middle ${cellBorderColor}`}><div className="flex items-center gap-2"><span className={`inline-flex min-w-[30px] items-center justify-center rounded-md px-2 py-1 text-[13px] font-semibold ${count > 0 ? 'bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400' : 'bg-slate-100 text-slate-400 dark:bg-slate-800 dark:text-slate-500'}`}>{count}</span><span className="text-[13px] text-slate-500 dark:text-slate-400">paiement{count > 1 ? 's' : ''}</span></div></td>
              <td className={`border px-5 py-2.5 align-middle ${cellBorderColor}`}><span className={`text-[13px] font-semibold ${globalTotal > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400 dark:text-slate-500'}`}>{formatMoney(globalTotal)}</span></td>
              <td className={`border px-5 py-2.5 align-middle ${cellBorderColor}`}><button type="button" onClick={() => onViewHistorique(employeeId)} disabled={!employeeId} className="inline-flex items-center gap-1.5 rounded-md bg-indigo-50 px-3 py-1.5 text-[13px] font-semibold text-indigo-600 transition-all hover:bg-indigo-100 hover:text-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-indigo-500/10 dark:text-indigo-400 dark:hover:bg-indigo-500/20 dark:hover:text-indigo-300"><Eye size={13} strokeWidth={2} />Voir détails</button></td>
            </tr>);
          })}
        </tbody>
      </table>
