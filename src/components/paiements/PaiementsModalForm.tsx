...
import { CreditCard, X, DollarSign, Calendar as CalendarIcon, Hash, FileText, Plus, Wallet, Building2 } from 'lucide-react';
import { useTheme } from '../../contexts/ThemeContext';
import DatePicker from 'react-datepicker';
import 'react-datepicker/dist/react-datepicker.css';

const COLORS = { light: { overlay: 'rgba(15, 23, 42, 0.55)', card: '#FFFFFF', header: '#FFFFFF', footer: '#F8FAFC', border: '#E2E8F0', input: '#FFFFFF', inputMuted: '#F8FAFC', text: '#202124', muted: '#5F6368', subMuted: '#80868B', primary: '#6366F1', primaryHover: '#4F46E5', primarySoft: '#EEF2FF', danger: '#D93025', dangerSoft: '#FCE8E6', success: '#188038' }, dark: { overlay: 'rgba(0, 0, 0, 0.72)', card: '#0F172A', header: '#0F172A', footer: '#0F172A', border: '#334155', input: '#0F172A', inputMuted: '#111827', text: '#F8FAFC', muted: '#CBD5E1', subMuted: '#94A3B8', primary: '#6366F1', primaryHover: '#818CF8', primarySoft: 'rgba(99, 102, 241, 0.12)', danger: '#F28B82', dangerSoft: 'rgba(242, 139, 130, 0.10)', success: '#81C995' } };
...
const FormField: React.FC<FormFieldProps> = ({ label, children, icon, required = false, className = '' }) => {
  ...
  return (<div className={`flex flex-col gap-1.5 ${className}`}><label className="flex items-center gap-1.5 text-[14.5px] font-medium" style={{ color: theme.muted }}>{icon && <span className="flex items-center justify-center" style={{ color: theme.subMuted }}>{icon}</span>}<span>{label}{required && <span className="ml-1" style={{ color: theme.danger }}>*</span>}</span></label>{children}</div>);
};

const inputBase = `w-full h-10 px-3 rounded-lg border text-[14.5px] font-medium outline-none transition-all duration-150 focus:ring-2`;
...
interface PaiementsModalFormProps { isOpen: boolean; onClose: () => void; onSubmit: (e: React.FormEvent<HTMLFormElement>) => void; editingPaiement: Paiement | null; employes: Employe[]; moisOptions: { value: number; label: string; }[]; annees: number[]; isDark?: boolean; }
...
const PaiementsModalForm: React.FC<PaiementsModalFormProps> = ({ isOpen, onClose, onSubmit, editingPaiement, employes, moisOptions, annees, isDark: propIsDark }) => {
  const { isDark: contextIsDark } = useTheme(); const isDark = propIsDark !== undefined ? propIsDark : contextIsDark; const theme = isDark ? COLORS.dark : COLORS.light;
  const formRef = useRef<HTMLFormElement>(null); const [isVisible, setIsVisible] = useState(false); const [selectedDate, setSelectedDate] = useState<Date | null>(null); const [amount, setAmount] = useState<number>(Number(editingPaiement?.montant || 0));

  // ⭐ FANITSIA MAJOR : Border Class
  const borderClass = isDark ? 'border-slate-700' : 'border-gray-300';

  const defaultDate = useMemo(() => { if (editingPaiement) return new Date(Number(editingPaiement.annee), Number(editingPaiement.mois) - 1, 1); const now = new Date(); return new Date(now.getFullYear(), now.getMonth(), 1); }, [editingPaiement]);
  ...
  useEffect(() => { if (!isOpen) return; setSelectedDate(defaultDate); setAmount(Number(editingPaiement?.montant || 0)); }, [isOpen, defaultDate, editingPaiement]);
  ...
  const inputClass = `${inputBase} ${isDark ? 'border-slate-700 bg-[#0F172A] text-slate-100 placeholder:text-slate-600 focus:border-[#8AB4F8] focus:ring-[#8AB4F8]/15' : 'border-gray-300 bg-white text-slate-900 placeholder:text-slate-400 focus:border-[#6366F1] focus:ring-[#6366F1]/15'}`;
  const selectedEmployee = useMemo(() => { if (!editingPaiement) return null; return employes.find(e => Number(e.id) === Number(editingPaiement.employe_id)) || null; }, [employes, editingPaiement]);
  const employeeName = editingPaiement ? `${editingPaiement.prenom || ''} ${editingPaiement.nom || ''}`.trim() : selectedEmployee ? `${selectedEmployee.prenom || ''} ${selectedEmployee.nom || ''}`.trim() : '—';
  ...
  return (<div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6 transition-opacity duration-200" style={{ background: theme.overlay, backdropFilter: 'blur(4px)' }} role="dialog" aria-modal="true" aria-labelledby="paiement-modal-title" onMouseDown={(e) => { if (e.target === e.currentTarget) handleClose(); }}>
          ...
          <div className="grid grid-cols-1 gap-7 lg:grid-cols-[220px_minmax(0,1fr)]">
            <aside>
              <div className={`sticky top-0 overflow-hidden rounded-xl border ${borderClass}`} style={{ background: isDark ? '#111827' : '#F8FAFC' }}>
                <div className={`flex items-center gap-2 border-b px-4 py-3 ${borderClass}`}>
                  <Wallet className="h-4 w-4" style={{ color: theme.primary }} />
                ...
                <div className="p-4">
                  <div className="space-y-3">
                    ...
                    {(editingPaiement?.poste || selectedEmployee?.poste) && <div className="flex items-center justify-between gap-3">
                      <span className="text-[14.5px]" style={{ color: theme.muted }}>Poste</span>
                      <span className="max-w-[120px] truncate text-right text-[14px] font-medium" style={{ color: theme.text }}>{editingPaiement?.poste || selectedEmployee?.poste}</span>
                    </div>}
                    <div className="flex items-center justify-between gap-3">
            ...
            <div className="min-w-0">
              <div className="grid grid-cols-1 gap-x-5 gap-y-5 md:grid-cols-2">
                <FormField label="Employé" required icon={<Building2 className="h-3.5 w-3.5" />} className="md:col-span-2">
                ...
                <FormField label="Période" required icon={<CalendarIcon className="h-3.5 w-3.5" />} className="md:col-span-2">
                  <DatePicker
                ...
                <FormField label="Date de paiement" icon={<CalendarIcon className="h-3.5 w-3.5" />}>
    ...
    <style>{`@keyframes modalIn { from { opacity: 0; transform: translateY(6px) scale(0.985); } to { opacity: 1; transform: translateY(0) scale(1); } } .custom-modal-scrollbar::-webkit-scrollbar { width: 6px; } .custom-modal-scrollbar::-webkit-scrollbar-track { background: transparent; } .custom-modal-scrollbar::-webkit-scrollbar-thumb { background: rgba(100, 116, 139, 0.25); border-radius: 999px; } .custom-modal-scrollbar::-webkit-scrollbar-thumb:hover { background: rgba(100, 116, 139, 0.4); } .react-datepicker-wrapper { width: 100%; } .react-datepicker-popper { z-index: 100000 !important; } .react-datepicker { border-radius: 12px !important; overflow: hidden; font-family: inherit !important; box-shadow: 0 20px 50px rgba(0,0,0,0.18) !important; } .react-datepicker__header { padding-top: 12px !important; } .react-datepicker__current-month { font-size: 14px !important; font-weight: 600 !important; } .react-datepicker__month-text { border-radius: 8px !important; margin: 4px !important; padding: 8px 4px !important; transition: all 120ms ease; } .react-datepicker__month-text:hover { background: #6366F1 !important; color: white !important; } .react-datepicker__month-text--selected, .react-datepicker__month-text--keyboard-selected { background: #6366F1 !important; color: white !important; font-weight: 600 !important; } .dark-datepicker-popper .react-datepicker, .dark-datepicker { background-color: #0F172A !important; border-color: #334155 !important; color: #F8FAFC !important; } .dark-datepicker-popper .react-datepicker__header { background-color: #111827 !important; border-color: #334155 !important; } .dark-datepicker-popper .react-datepicker__current-month, .dark-datepicker-popper .react-datepicker__month-text { color: #F8FAFC !important; } .dark-datepicker-popper .react-datepicker__month-text:hover { background: #6366F1 !important; color: #FFFFFF !important; } .dark-datepicker-popper .react-datepicker__month-text--selected, .dark-datepicker-popper .react-datepicker__month-text--keyboard-selected { background: #6366F1 !important; color: #FFFFFF !important; } .dark-datepicker-popper .react-datepicker__navigation-icon::before { border-color: #CBD5E1 !important; } .light-datepicker-popper .react-datepicker, .light-datepicker { background-color: #FFFFFF !important; border-color: #E2E8F0 !important; } .light-datepicker-popper .react-datepicker__header { background-color: #F8FAFC !important; border-color: #E2E8F0 !important; } .light-datepicker-popper .react-datepicker__current-month, .light-datepicker-popper .react-datepicker__month-text { color: #0F172A !important; } .light-datepicker-popper .react-datepicker__month-text:hover { background: #6366F1 !important; color: #FFFFFF !important; } .light-datepicker-popper .react-datepicker__month-text--selected, .light-datepicker-popper .react-datepicker__month-text--keyboard-selected { background: #6366F1 !important; color: #FFFFFF !important; }`}</style>
  </div>);
};
