          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-3 py-2.5 dark:border-slate-700 dark:bg-slate-800/50">
              <div className="flex items-center gap-2.5">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-indigo-100 bg-indigo-50 text-indigo-600 shadow-sm transition-all duration-150 group-hover:border-indigo-200 group-hover:bg-indigo-100 group-hover:shadow-md dark:border-indigo-500/10 dark:bg-indigo-500/10 dark:text-indigo-400 dark:group-hover:bg-indigo-500/15">
                  {hasImage ? <img src={fournisseur.image!} alt={fournisseur.nom} loading="lazy" className="h-full w-full object-cover rounded-lg" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} /> : <span className="text-[14px] font-bold">{initiales}</span>}
                </div>
                <div className="min-w-0">
                  <div className="truncate text-[14px] font-semibold text-slate-800 transition-colors group-hover:text-indigo-600 dark:text-slate-200 dark:group-hover:text-indigo-400">{fournisseur.nom || 'Fournisseur inconnu'}</div>
                  <div className="mt-0.5 text-[11px] text-slate-400 dark:text-slate-500">ID #{String(fournisseur.id).padStart(3, '0')}</div>
                </div>
              </div>
              <div className="flex flex-col items-end text-[11px] text-slate-400 dark:text-slate-500">
                <div className="flex items-center gap-1">
                  <CalendarDays size={11} />
                  <span>{fournisseur.created_at ? new Date(fournisseur.created_at).toLocaleDateString('fr-FR') : '—'}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-1 flex-col p-3 pt-2.5 space-y-1">
              {fournisseur.contact ? <div className="flex items-center gap-2 text-[13px] text-slate-600 dark:text-slate-300"><span className="text-slate-400 dark:text-slate-500">Contact :</span><span className="truncate font-medium">{fournisseur.contact}</span></div> : <span className="text-[13px] italic text-slate-400 dark:text-slate-500">Aucun contact</span>}
              {fournisseur.telephone ? <div className="flex items-center gap-2 text-[13px] text-slate-600 dark:text-slate-300"><Phone size={13} className="shrink-0" /><span className="truncate">{fournisseur.telephone}</span></div> : <span className="text-[13px] italic text-slate-400 dark:text-slate-500">—</span>}
              {fournisseur.email ? <div className="flex items-center gap-2 text-[13px] text-slate-600 dark:text-slate-300"><Mail size={13} className="shrink-0" /><span className="truncate">{fournisseur.email}</span></div> : <span className="text-[13px] italic text-slate-400 dark:text-slate-500">—</span>}
            </div>

            {/* ⭐ ACTIONS COMPACT (H-8) */}
            <div className="mt-2 flex items-center gap-1.5 border-t border-slate-200 px-3 py-2.5 dark:border-slate-700" onClick={(e) => e.stopPropagation()}>
              <button type="button" title="Voir les produits" onClick={() => navigate(`/produits?fournisseur=${fournisseur.id}`)} className="flex h-8 flex-1 items-center justify-center rounded-md bg-slate-100 text-slate-600 transition hover:bg-indigo-100 hover:text-indigo-600 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-indigo-500/10 dark:hover:text-indigo-400"><Package size={14} className="mx-auto" /></button>
