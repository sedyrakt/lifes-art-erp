          ...
          >
            <div className="relative h-48 w-full overflow-hidden bg-slate-100 dark:bg-slate-800">
              {imageUrl && !hasImageError ? (
                ...
                <div className="flex h-full w-full items-center justify-center bg-indigo-50 dark:bg-indigo-500/10"><span className="text-4xl font-bold text-indigo-600 dark:text-indigo-400">{initiales}</span></div>
              )}
              <div className="absolute right-2 top-2">
                <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[14.5px] font-semibold ${getStatusColor(employe.status)}`}>{getStatusIcon(employe.status)}{statusLabel}</span>
              </div>
            </div>

            <div className="flex flex-1 flex-col p-4">
              <div className="mb-1 flex items-start justify-between">
                <h4 className="text-[14.5px] font-semibold text-slate-900 line-clamp-2 dark:text-slate-100" title={`${employe.prenom} ${employe.nom}`}>{employe.prenom} {employe.nom}</h4>
                <div className="text-[14.5px] text-slate-400 dark:text-slate-500"><Calendar size={14} className="inline mr-1" />{employe.date_embauche ? new Date(employe.date_embauche).toLocaleDateString('fr-FR') : '—'}</div>
              </div>
              <div className="space-y-0.5 text-[14.5px] text-slate-500 dark:text-slate-400">
                <div className="flex items-center gap-2 truncate"><User size={14} className="shrink-0" /><span className="truncate font-medium text-slate-700 dark:text-slate-300">{employe.poste || 'Poste non défini'}</span></div>
              ...
              <div className="mt-2 space-y-1 text-[14.5px] text-slate-500 dark:text-slate-400">
                {employe.email && <div className="flex items-center gap-2 truncate"><Mail size={14} className="shrink-0" /><span className="truncate" title={employe.email}>{employe.email}</span></div>}
                {employe.telephone && <div className="flex items-center gap-2 truncate"><Phone size={14} className="shrink-0" /><span className="truncate">{employe.telephone}</span></div>}
              ...
              <div className="mt-3 flex items-center justify-between border-t border-slate-200 pt-3 dark:border-slate-700">
                <div className="flex flex-col"><span className="text-[14.5px] text-slate-400 dark:text-slate-500">Salaire mensuel</span><span className="text-[15px] font-bold text-indigo-600 dark:text-indigo-400">{formatMoney(employe.salaire)}</span></div>
              </div>
              <div className="mt-4 flex items-center gap-2 border-t border-slate-200 pt-3 dark:border-slate-700" onClick={(e) => e.stopPropagation()}>
                <button type="button" title="Modifier" onClick={() => onEdit(employe)} className="flex-1 rounded-lg bg-slate-100 px-3 py-2 text-slate-600 transition hover:bg-amber-100 hover:text-amber-600 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-amber-500/10 dark:hover:text-amber-400"><Edit size={16} className="mx-auto" /></button>
                <button type="button" title="Payer salaire" onClick={() => onPaiement(employe)} className="flex-1 rounded-lg bg-emerald-100 px-3 py-2 text-emerald-700 transition hover:bg-emerald-200 hover:text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400 dark:hover:bg-emerald-800/50"><Wallet size={16} className="mx-auto" /></button>
                <button type="button" title="Historique" onClick={() => onHistorique(employe)} className="flex-1 rounded-lg bg-indigo-100 px-3 py-2 text-indigo-700 transition hover:bg-indigo-200 hover:text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-400 dark:hover:bg-indigo-800/50"><History size={16} className="mx-auto" /></button>
                <button type="button" title="Supprimer" onClick={() => onDelete(employe.id, employe.image)} className="flex-1 rounded-lg bg-rose-100 px-3 py-2 text-rose-700 transition hover:bg-rose-200 hover:text-rose-800 dark:bg-rose-900/30 dark:text-rose-400 dark:hover:bg-rose-800/50"><Trash2 size={16} className="mx-auto" /></button>
              </div>
            </div>
          </div>
        );
