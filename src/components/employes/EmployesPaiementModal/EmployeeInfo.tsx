...
const EmployeeInfo: React.FC<EmployeeInfoProps> = ({
  ...
  return (
    <div className="rounded-xl border overflow-hidden" style={{ background: theme.card, borderColor: theme.border }}>
      {/* HEADER */}
      <div className="px-4 py-3 border-b" style={{ background: theme.headerBg, borderColor: theme.border }}>
      ...
      <div className="p-4">
        <div className="flex items-center gap-3">
          <div className="w-14 h-14 rounded-xl overflow-hidden border flex items-center justify-center shrink-0"
            style={{ borderColor: theme.border, background: theme.primaryBg }}>
            {imageUrl ? (
              <img src={imageUrl} alt={`${employe.prenom} ${employe.nom}`} className="w-full h-full object-cover" />
            ) : (
              <span className="font-extrabold text-lg" style={{ color: theme.primary }}>
                {getInitiales(employe.prenom, employe.nom)}
              </span>
            )}
          </div>

          <div className="min-w-0">
            <p className="text-sm font-extrabold truncate" style={{ color: theme.text }}>
              {employe.prenom} {employe.nom}
            </p>
            <p className="text-xs font-semibold truncate mt-1" style={{ color: theme.primary }}>
        ...
        <div className="mt-4 pt-4 border-t space-y-3" style={{ borderColor: theme.border }}>
          <div className="flex items-center justify-between gap-3">
            <span className="text-xs font-medium" style={{ color: theme.muted }}>Département</span>
          ...
          <div className="flex items-center justify-between gap-3">
            <span className="text-xs font-medium" style={{ color: theme.muted }}>Embauche</span>
