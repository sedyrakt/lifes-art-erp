// src/components/achats/AchatsPagination.tsx
...
interface AchatsPaginationProps { 
  currentPage: number; 
  totalPages: number; 
  totalItems: number; 
...
const AchatsPagination: React.FC<AchatsPaginationProps> = ({ 
  ...
  const pageNumbers = useMemo(() => {
    ...
    // Namboarina kely mba hisehoana ilay boky 1 rehefa tokana ny pejy
  ...
  // ⚠️ ESORY ILAY CONDITION MAHAVOA ny "if (totalPages <= 1) return null;"
  ...
  return (
    <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4 px-2 w-full">
      <div className="text-xs font-bold uppercase tracking-wider" style={{ color: theme.muted }}>
        Total : <span style={{ color: theme.text }} className="font-black">{totalItems}</span> achat{totalItems > 1 ? 's' : ''}
      </div>
...
export default AchatsPagination;