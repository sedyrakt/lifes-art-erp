...
// ⭐ PRODUCTION READY - FIX DOUBLE SAVE DIALOG
// ============================================================
...
const COLORS={
  light:{
    overlay:'rgba(15,23,42,.36)',card:'#FFF',surface:'#FFF',surfaceSoft:'#F8FAFC',footer:'#F8FAFC',border:'#E2E8F0',
    text:'#0F172A',muted:'#64748B',subMuted:'#94A3B8',primary:'#4F46E5',primaryHover:'#4338CA',
    primaryBg:'rgba(79,70,229,.07)',primaryBorder:'rgba(79,70,229,.15)',success:'#059669',
    successBg:'rgba(16,185,129,.08)',successBorder:'rgba(16,185,129,.16)',warning:'#D97706',
    warningBg:'rgba(245,158,11,.08)',warningBorder:'rgba(245,158,11,.16)',danger:'#E11D48',
    dangerBg:'rgba(244,63,94,.07)',dangerBorder:'rgba(244,63,94,.15)'
  },
  dark:{
    overlay:'rgba(7,4,4,.66)',card:'#0F172A',surface:'#0F172A',surfaceSoft:'#111C30',footer:'#0F172A',border:'#334155',
    text:'#F8FAFC',muted:'#94A3B8',subMuted:'#64748B',primary:'#818CF8',primaryHover:'#6366F1',
    primaryBg:'rgba(99,102,241,.12)',primaryBorder:'rgba(129,140,248,.20)',success:'#34D399',
    successBg:'rgba(16,185,129,.11)',successBorder:'rgba(52,211,153,.18)',warning:'#FBBF24',
    warningBg:'rgba(245,158,11,.11)',warningBorder:'rgba(251,191,36,.18)',danger:'#FB7185',
    dangerBg:'rgba(244,63,94,.11)',dangerBorder:'rgba(251,113,133,.18)'
  }
};
...
const CompanySettingsModal:React.FC<ExtendedCompanySettingsModalProps>=({
  ...
  const {formData,imagePreview,loading,errors,savingImage,handleChange,handleImageChange,handleRemoveImage,handleDrop,handleDragOver,handleGenerate}=useCompanySettings(initialData,onSave,onGenerate);
  ...
  useEffect(()=>{
    ...
    const handleKeyDown=(e:KeyboardEvent)=>{
      if(e.key==='Escape'&&!isGenerating&&!loading&&!savingImage)onClose();
    };
    window.addEventListener('keydown',handleKeyDown);
    return()=>window.removeEventListener('keydown',handleKeyDown);
  },[isOpen,isGenerating,loading,savingImage,onClose]);
  ...
  const handleGenerateWithClose=async()=>{
    ...
    try{
      ...
      if(result?.success){
        ...
        onPDFGenerated?.(true,result?.filePath);
        onClose();
        return;
      }
  ...
  return(
    <>
      <div
        ...
        onMouseDown={e=>{if(e.target===e.currentTarget&&!isGenerating&&!loading&&!savingImage)onClose();}}
      >
        <div
          className={`relative flex w-full max-w-[950px] max-h-[85vh] flex-col overflow-hidden rounded-2xl border shadow-[0_24px_80px_rgba(0,0,0,.22)] transition-all duration-200 ${isVisible?'translate-y-0 scale-100':'translate-y-2 scale-[.985]'}`}
        ...
        >
          ...
          <header className="flex shrink-0 items-center justify-between gap-4 border-b px-5 py-4 sm:px-6" style={{borderColor:theme.border,background:theme.surface}}>
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                <h2 id="company-settings-modal-title" className="truncate text-[16px] font-semibold tracking-tight sm:text-[17px]" style={{color:theme.text}}>
                  Paramètres de l'entreprise
                </h2>
                <p className="mt-0.5 truncate text-[12px]" style={{color:theme.muted}}>
            ...
            <button
              ...
              onClick={onClose}
              aria-label="Fermer"
              disabled={isGenerating||loading||savingImage}
              className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-500 transition-all hover:bg-slate-100 hover:text-slate-900 active:scale-95 disabled:pointer-events-none disabled:opacity-50 dark:text-slate-400 dark:hover:bg-white/[.06] dark:hover:text-white"
            >
          ...
          <form
            className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 py-5 sm:px-6 sm:py-6 custom-company-scrollbar"
          ...
          <div className="flex shrink-0 items-center justify-between gap-3 border-t px-5 py-3 sm:px-6" style={{background:theme.footer,borderColor:theme.border}}>
            ...
            <div className="ml-auto flex items-center gap-2">
              <button
                ...
                onClick={onClose}
                disabled={isGenerating||loading||savingImage}
                className="h-9 rounded-lg border px-4 text-[13px] font-medium transition-all hover:bg-slate-100 active:scale-[.98] disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-white/[.05]"
              ...
              <button
                ...
                className="flex h-9 items-center gap-2 rounded-lg px-4 text-[13px] font-semibold text-white shadow-sm transition-all hover:shadow-md active:scale-[.98] disabled:cursor-not-allowed disabled:opacity-50"
              ...
              >
                {loading||savingImage||isGenerating?(
                  <>
                    ...
                    {savingImage?'Sauvegarde image...':'Génération...'}
                  </>
                ):(
                  'Générer la facture'
                )}
          ...
          <style>{`
            .custom-company-scrollbar{scrollbar-width:thin;scrollbar-color:rgba(100,116,139,.28) transparent}
            ...
            .custom-company-scrollbar::-webkit-scrollbar-thumb{background:rgba(100,116,139,.22);border-radius:999px}
            .custom-company-scrollbar::-webkit-scrollbar-thumb:hover{background:rgba(100,116,139,.38)}
