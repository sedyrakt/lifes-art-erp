...
const COLORS = {
  light: {
    ...
    border: '#E2E8F0', input: '#FFFFFF', inputMuted: '#F8FAFC', text: '#202124', muted: '#5F6368',
    subMuted: '#80868B', primary: '#6366F1', primaryHover: '#185ABC', primarySoft: '#E8F0FE',
    danger: '#D93025', dangerSoft: '#FCE8E6', success: '#188038'
  },
  dark: {
    overlay: 'rgba(0, 0, 0, 0.72)', card: '#0F172A', header: '#0F172A', footer: '#0F172A',
    border: '#334155', input: '#0F172A', inputMuted: '#111827', text: '#F8FAFC', muted: '#CBD5E1',
    subMuted: '#94A3B8', primary: '#6366F1', primaryHover: '#A8C7FA', primarySoft: 'rgba(138, 180, 248, 0.12)',
    danger: '#F28B82', dangerSoft: 'rgba(242, 139, 130, 0.10)', success: '#81C995'
  },
};
...
const FormField: React.FC<{ label: string; children: React.ReactNode; icon?: React.ReactNode; required?: boolean; className?: string; isDark: boolean; style?: any }> = ({ label, children, icon, required = false, className = '', isDark, style }) => {
...
const CompanySettingsForm: React.FC<CompanySettingsFormProps> = ({
  ...
  const isValidImage = Boolean(imagePreview && imagePreview.trim() !== '');
  const inputClass = (field?: string) =>
    `${inputBase} ${
      isDark
        ? 'border-slate-700 bg-[#0F172A] text-slate-100 placeholder:text-slate-600 focus:border-[#8AB4F8] focus:ring-[#8AB4F8]/15'
        : 'border-slate-300 bg-white text-slate-900 placeholder:text-slate-400 focus:border-[#1A73E8] focus:ring-[#1A73E8]/15'
      ...
      <aside className="w-full shrink-0 lg:w-[220px]">
        <div className="overflow-hidden rounded-xl border" style={{ background: theme.card, borderColor: theme.border }}>
          <div className="border-b px-4 py-3" style={{ borderColor: theme.border, background: isDark ? '#0F172A' : '#F8FAFC' }}>
            ...
            <div className="mt-0.5 text-[12px]" style={{ color: theme.subMuted }}>PNG ou JPG · 5 MB max.</div>
          </div>
          <div className="p-3">
            {isValidImage ? (
              <div className="space-y-2.5">
                <div className="group relative flex h-[145px] items-center justify-center overflow-hidden rounded-lg border" style={{ background: theme.input, borderColor: theme.border }}>
                  ...
                  <div className="absolute inset-0 flex items-center justify-center gap-2 bg-slate-950/55 opacity-0 transition-opacity duration-200 group-hover:opacity-100">
                    ...
                    <button type="button" onClick={onRemoveImage} className="flex h-9 w-9 items-center justify-center rounded-lg bg-rose-600 text-white shadow-lg transition-transform hover:scale-105" title="Supprimer">
                      <Trash2 size={16} />
                    </button>
                  </div>
                  {savingImage && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-white/85 backdrop-blur-sm dark:bg-slate-950/85">
                      ...
                      <span className="text-[12px] font-medium" style={{ color: theme.muted }}>Téléchargement en cours...</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div onDrop={onDrop} onDragOver={onDragOver} onClick={() => fileInputRef.current?.click()}
                className="group flex h-[145px] cursor-pointer flex-col items-center justify-center rounded-lg border border-dashed px-4 text-center transition-all duration-200 hover:border-indigo-400 hover:bg-indigo-50/40 dark:hover:border-indigo-500 dark:hover:bg-indigo-500/5"
                style={{ borderColor: theme.border, background: isDark ? 'rgba(255,255,255,0.015)' : '#FAFAFA' }}>
                <div className="mb-2 flex h-10 w-10 items-center justify-center rounded-lg transition-transform duration-200 group-hover:scale-105" style={{ background: theme.primarySoft, color: theme.primary }}>
                  <ImageIcon size={20} />
                </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2">
          ...
          <FormField label="Mode de paiement" icon={<CreditCard size={14} />} isDark={isDark} className="border-r border-b px-4 py-3.5" style={{ borderColor: theme.border }}>
            <div className="relative">
              <button type="button" onClick={() => setIsPaymentMethodOpen(prev => !prev)}
                className="flex h-10 w-full items-center justify-between rounded-lg border px-3 text-left text-[14px] font-medium outline-none transition-all"
                style={{ background: theme.input, color: theme.text, borderColor: isPaymentMethodOpen ? theme.primary : theme.border }}>
                <span className="flex min-w-0 items-center gap-2">
                  {SelectedPaymentIcon && <SelectedPaymentIcon size={16} style={{ color: theme.primary }} />}
                  <span className="truncate">{selectedPayment?.label || 'Espèces'}</span>
                </span>
                <ChevronDown size={15} className={`shrink-0 transition-transform duration-150 ${isPaymentMethodOpen ? 'rotate-180' : ''}`} style={{ color: theme.subMuted }} />
              </button>
              {isPaymentMethodOpen && (
                <div className="absolute left-0 right-0 top-full z-50 mt-1 overflow-hidden rounded-lg border p-1 shadow-xl" style={{ background: isDark ? '#0F172A' : '#FFFFFF', borderColor: theme.border }}>
                  {paymentMethods.map(method => {
                    const Icon = method.icon;
                    const isSelected = formData.paymentMethod === method.value;
                    return (
                      <button key={method.value} type="button" onClick={() => selectPaymentMethod(method.value)}
                        className="flex w-full items-center gap-2.5 rounded-md px-2.5 py-2 text-left text-[14px] transition-colors hover:bg-slate-100 dark:hover:bg-slate-800"
                        style={{ color: isSelected ? theme.primary : theme.text, background: isSelected ? theme.primarySoft : 'transparent' }}>
                        <Icon size={15} className="shrink-0" style={{ color: isSelected ? theme.primary : theme.subMuted }} />
                        <span className="flex-1">{method.label}</span>
                        {isSelected && <Check size={14} strokeWidth={2.5} style={{ color: theme.primary }} />}
                      </button>
                    );
                  })}
                </div>
          ...
          <FormField label="Conditions de paiement" icon={<Clock size={14} />} isDark={isDark} className="border-b px-4 py-3.5" style={{ borderColor: theme.border }}>
            <div className="relative">
              <select name="paymentTerms" value={formData.paymentTerms || 'Sous 30 jours'} onChange={onChange}
                className="h-10 w-full appearance-none rounded-lg border px-3 pr-9 text-[14px] font-medium outline-none"
                style={{ background: theme.input, color: theme.text, borderColor: theme.border }}>
