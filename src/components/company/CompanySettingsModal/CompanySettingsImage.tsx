...
// ⭐ UNIFIED DESIGN: Mampiasa ny COLORS sy inputBase unified
// ============================================================
      ...
      <div className="p-4">
        {isValidImage ? (
          <div className="space-y-3">
            <div
              className="group relative flex h-[150px] w-full items-center justify-center overflow-hidden rounded-xl border"
              style={{ background: theme.inputMuted, borderColor: theme.border }}
            >
              <img
                src={imagePreview}
              ...
              <div className="pointer-events-none absolute inset-0 flex items-center justify-center gap-2 bg-slate-950/45 opacity-0 backdrop-blur-[2px] transition-opacity duration-200 group-hover:pointer-events-auto group-hover:opacity-100">
                <button
                  ...
                  className="inline-flex h-9 items-center gap-2 rounded-lg bg-white px-3 text-[14px] font-medium text-slate-700 shadow-lg transition-all hover:bg-slate-50 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"
                >
                ...
                <button
                  ...
                  disabled={savingImage}
                  className="inline-flex h-9 items-center gap-2 rounded-lg bg-rose-600 px-3 text-[14px] font-medium text-white shadow-lg transition-all hover:bg-rose-700 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"
                >
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <button
                ...
                onClick={handleReplace}
                disabled={savingImage}
                className="text-[14px] font-medium transition-colors disabled:cursor-not-allowed disabled:opacity-50"
          ...
          <div
            ...
            className={`group flex min-h-[170px] w-full cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed px-5 py-6 text-center transition-all duration-200 ${
              isDragging
                ? 'border-indigo-500 bg-indigo-50/70 dark:border-indigo-400 dark:bg-indigo-500/10'
                : 'hover:border-indigo-400 hover:bg-indigo-50/30 dark:hover:border-indigo-500/60 dark:hover:bg-indigo-500/5'
            }`}
            style={{ borderColor: isDragging ? undefined : (isDark ? '#334155' : '#CBD5E1') }}
          >
            <div
              className="mb-3 flex h-11 w-11 items-center justify-center rounded-xl transition-all duration-200 group-hover:scale-105"
              style={{ background: theme.primarySoft, color: theme.primary }}
            >
