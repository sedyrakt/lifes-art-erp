...
// ⭐ PRO / PREMIUM RAPPORTS CHARTS - COMPACT
// ⭐ FONT SIZE 13px-15px
// ============================================================
...
const sum = (array: any[], key: string): number =>
  array.reduce((total, item) => total + toNumber(item?.[key]), 0);

const first = (array: any[], key: string, fallback = 'Aucun'): string =>
  array?.[0]?.[key] || fallback;

const firstNum = (array: any[], key: string): number =>
  toNumber(array?.[0]?.[key]);

const hasData = (array: any[], key: string): boolean =>
  array.some((item) => toNumber(item?.[key]) > 0);
...
const RapportsCharts: React.FC<RapportsChartsProps> = ({
  ...
  const categoryChartValues = useMemo(() => {
    ...
    return hasSalesValue
      ? normalizedCategories.map((category) => category.valeur_vente)
      : normalizedCategories.map((category) => category.total_produits);
  }, [normalizedCategories]);
  ...
  const kpis = useMemo(() => {
    ...
    switch (activeTab) {
      case 'ventes': {
        ...
        return { items: [
          C("Chiffre d'affaires", ca, 'money'),
          C('Commandes', cmd, 'number'),
          C('Panier moyen', cmd > 0 ? ca / cmd : 0, 'money'),
          C('Vente max', Math.max(...ventesParMois.map((i) => toNumber(i?.total_ventes)), 0), 'money'),
        ]};
      }
      case 'stock': {
        ...
        return { items: [
          C('Entrées', ent, 'number'),
          C('Sorties', sor, 'number'),
          C('Solde', ent - sor, 'number'),
          C('En stock', stockStatus.en_stock, 'number'),
        ]};
      }
      case 'top':
        return { items: [
          C('Top produit', first(topProduits, 'nom'), 'text'),
          C('Quantité vendue', firstNum(topProduits, 'total_vendu'), 'number'),
          C('Total vendu', sum(topProduits, 'total_vendu'), 'number'),
          C('Total ventes', sum(topProduits, 'total_ventes'), 'money'),
        ]};
      case 'categories': {
        ...
        return { items: [
          C('Catégories', normalizedCategories.length, 'number'),
          C('Produits', totalProd, 'number'),
          C('Stock total', totalStock, 'number'),
          C('Top catégorie', topPct > 0 ? `${topCategory} (${topPct.toFixed(1)}%)` : topCategory, 'text'),
        ]};
      }
      case 'clients':
        return { items: [
          C('Clients', topClients.length, 'number'),
          C('Total achats', sum(topClients, 'total_achats'), 'money'),
          C('Top client', first(topClients, 'client_nom'), 'text'),
          C('Achats top client', firstNum(topClients, 'total_achats'), 'money'),
        ]};
      case 'depenses':
        return { items: [
          C('Total dépenses', sum(depensesParCategorie, 'total'), 'money'),
          C('Catégories', depensesParCategorie.length, 'number'),
          C('Top catégorie', first(depensesParCategorie, 'categorie'), 'text'),
          C('Montant top', firstNum(depensesParCategorie, 'total'), 'money'),
        ]};
      case 'commandes':
        return { items: [
          C('Total commandes', sum(commandesStatut, 'nb'), 'number'),
          C('Statuts', commandesStatut.length, 'number'),
          C('Top statut', first(commandesStatut, 'statut'), 'text'),
          C('Nb top statut', firstNum(commandesStatut, 'nb'), 'number'),
        ]};
  ...
  const salesChart = useMemo(() => {
    ...
    if (granularity === 'jour' || granularity === 'semaine') {
      ...
      labels = ventesParMois.map((item) => {
        try {
          const parsed = parseISO(item?.jour);
          return isValid(parsed) ? format(parsed, fmt, { locale: fr }) : 'N/A';
        } catch { return 'N/A'; }
      });
  ...
  const ventesChartData = useMemo(() => ({
    ...
    datasets: [{
      ...
      backgroundColor: (ctx: any) => {
        ...
        gradient.addColorStop(0, 'rgba(99,91,255,0.42)');
  ...
  const stockLabels = useMemo(() =>
    [...new Set([...entreesStock.map((i) => i?.date), ...sortiesStock.map((i) => i?.date)])]
      .filter(Boolean)
      .sort(),
  ...
  const commandesData = useMemo(() => ({
    ...
    datasets: [{
      ...
      data: commandesStatut.length ? commandesStatut.map((item) => toNumber(item?.nb)) : [0],
      backgroundColor: [theme.primary, theme.green, theme.purple, theme.cyan, isDark ? '#4F46E5' : '#818CF8'],
      borderWidth: 0,
      borderRadius: 6,
      maxBarThickness: 48,
    ...
    return {
      ...
      scales: {
        x: {
          ...
          ticks: { color: chartColors.text, font: { size: 12 }, maxTicksLimit: 7 },
        },
        y: {
          beginAtZero: true,
          border: { display: false },
          grid: { color: chartColors.grid, drawBorder: false },
          ticks: { color: chartColors.text, font: { size: 12 }, padding: 8, callback: (value: any) => formatter(Number(value)) },
        },
      },
  ...
  const ChartCard = ({ title, subtitle, children }: { title: string; subtitle?: string; children: React.ReactNode }) => (
    <section className={`overflow-hidden rounded-xl border bg-white dark:bg-[#111c30] ${isDark ? 'border-white/[0.055] shadow-[0_12px_40px_rgba(0,0,0,0.18)]' : 'border-slate-200 shadow-[0_1px_2px_rgba(15,23,42,0.04)]'}`}>
      <div className={`flex items-center justify-between gap-4 border-b px-4 py-3 ${isDark ? 'border-white/[0.055]' : 'border-slate-200'}`}>
        <div className="min-w-0">
          <h2 className="truncate text-[15px] font-semibold text-slate-900 dark:text-slate-100">{title}</h2>
          {subtitle && <p className="mt-0.5 truncate text-[13px] text-slate-500 dark:text-slate-400">{subtitle}</p>}
        </div>
      </div>
        ...
        return (
          <ChartCard title="Évolution des ventes" subtitle={`Chiffre d'affaires — ${granularity}`}>
            <div className="h-[280px] sm:h-[300px]">
              {salesChart.values.some((value) => value > 0) ? (
                <Line data={ventesChartData} options={getChartOptions('ventes')} />
              ) : (
                <EmptyState message="Aucune donnée de vente pour cette période" />
              )}
            </div>
          </ChartCard>
        );
      ...
      case 'top':
        return (
          <ChartCard title="Top 5 produits" subtitle="Produits les plus vendus">
            <div className="h-[280px] sm:h-[300px]">
              {hasData(topProduits, 'total_ventes') ? (
                <Bar data={topProduitsData} options={getBarOptions('top')} />
              ) : (
                <EmptyState message="Aucune donnée de vente" />
              )}
            </div>
          </ChartCard>
        );
      case 'categories': {
        ...
        return (
          <ChartCard title="Répartition des ventes" subtitle="Chiffre d'affaires par catégorie">
            {!hasCatData ? (
              <EmptyState message="Aucune vente par catégorie" />
            ) : (
              <div className="grid grid-cols-1 items-center gap-5 px-2 pb-2 pt-1 sm:grid-cols-[190px_1fr]">
                <div className="relative mx-auto h-[220px] w-[220px]">
                  <Doughnut
                    data={{
                      labels: normalizedCategories.map((category) => category.nom),
                      datasets: [{
                        data: categoryChartValues,
                        backgroundColor: normalizedCategories.map((_, index) => catColors[index % catColors.length]),
                  ...
                  <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
                    <span className="max-w-[150px] truncate text-center text-[15px] font-bold tracking-tight" style={{ color: theme.text }}>
                      {formatMoney(categorySalesTotal)}
                    </span>
                    <span className="mt-1 text-[13px] font-medium" style={{ color: theme.subtle }}>Total ventes</span>
                  </div>
                </div>
                    ...
                    return (
                      <div key={category.id || `${category.nom}-${index}`} className="group flex items-center justify-between gap-3">
                        <div className="flex min-w-0 items-center gap-2">
                          <span className="h-2.5 w-2.5 shrink-0 rounded-[3px]" style={{ backgroundColor: catColors[index % catColors.length] }} />
                          <span className="truncate text-[13px] font-medium" style={{ color: theme.muted }}>{category.nom}</span>
                        </div>
                        <div className="flex shrink-0 items-center gap-2">
                          <span className="text-[13px] font-semibold" style={{ color: theme.text }}>{formatMoney(value)}</span>
                          <span className="w-[38px] text-right text-[13px] font-medium" style={{ color: theme.subtle }}>{percentage.toFixed(1)}%</span>
                        </div>
                      </div>
                    );
                  })}
      ...
      case 'commandes':
        return (
          <ChartCard title="Commandes par statut" subtitle="Répartition des commandes">
            <div className="h-[280px] sm:h-[300px]">
              {hasData(commandesStatut, 'nb') ? (
                <Bar data={commandesData} options={getBarOptions('commandes')} />
              ) : (
                <EmptyState message="Aucune commande disponible" />
              )}
            </div>
          </ChartCard>
        );
        ...
        <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
          {kpis.items.map((item, index) => (
            <div
              ...
              className="group relative min-w-0 overflow-hidden rounded-xl border border-slate-200 bg-white px-4 py-3 transition-all duration-300 hover:-translate-y-[2px] hover:shadow-[0_4px_12px_rgba(15,23,42,0.06)] dark:border-white/[0.055] dark:bg-[#111c30] dark:shadow-[0_10px_35px_rgba(0,0,0,0.20)]"
            >
              <p className="text-[13px] font-semibold uppercase tracking-[0.08em] text-slate-400">{item.label}</p>
              <p className="mt-1 truncate text-[20px] font-bold tracking-tight text-slate-900 dark:text-white">
