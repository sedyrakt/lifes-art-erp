...
export default function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 dark:bg-[#0A0F1E] dark:text-slate-100">
      ...
      <div className={`flex min-h-screen flex-col overflow-hidden transition-all duration-300 ${isCollapsed ? 'ml-[72px] pl-3' : 'ml-[260px] pl-0'}`}>
        {/* ⭐ FIX: Header mifanaraka amin'ny stats cards (dark: #111c30) */}
        <header className="sticky top-0 z-40 flex shrink-0 items-center justify-between border-b border-slate-200 bg-white px-5 py-3 backdrop-blur-xl transition-colors dark:border-white/[0.06] dark:bg-[#111c30]/95">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button type="button" title="Notifications" className="relative flex h-9 w-9 items-center justify-center rounded-lg text-slate-500 transition-all hover:bg-indigo-50 hover:text-indigo-600 dark:text-slate-400 dark:hover:bg-indigo-500/10 dark:hover:text-indigo-400"><Bell size={18} /><span className="absolute right-2 top-2 h-1.5 w-1.5 rounded-full bg-rose-500 ring-2 ring-white dark:ring-[#111c30]" /></button>
            <div className="relative ml-1">
              <button type="button" onClick={() => setIsDropdownOpen(prev => !prev)} className="flex items-center gap-2.5 rounded-xl border border-transparent px-2 py-1 transition-all hover:border-slate-200 hover:bg-slate-50 dark:hover:border-white/[0.08] dark:hover:bg-white/[0.04]">
                ...
                <div className="hidden min-w-0 text-left sm:block">
                  ...
                  <p className="max-w-[130px] truncate text-[13px] font-medium text-slate-500 dark:text-slate-400">{user?.role || 'Administrateur'}</p>
                </div>
              ...
              {isDropdownOpen && (
                <>
                  ...
                  {/* ⭐ FIX: Dropdown Light Mode (bg-white) + Dark Mode (bg-[#111c30]) */}
                  <div className="absolute right-0 z-20 mt-2 w-[280px] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_12px_40px_rgba(15,23,42,0.12)] animate-in fade-in zoom-in-95 duration-150 dark:border-white/[0.08] dark:bg-[#111c30] dark:shadow-2xl">
                    {/* ⭐ FIX: Header dropdown - Light (bg-slate-50) + Dark (bg-white/[0.02]) */}
                    <div className="border-b border-slate-200 bg-slate-50 px-4 py-4 dark:border-white/[0.06] dark:bg-white/[0.02]">
                      <div className="flex items-center gap-3">
                        {renderAvatarContent('lg')}
                        <div className="min-w-0 flex-1">
                          {/* ⭐ FIX: Text Light (text-slate-900) + Dark (text-white) */}
                          ...
                          <p className="mt-0.5 truncate text-[13px] text-slate-500 dark:text-slate-400">{user?.email || 'contact@lifesart.mg'}</p>
                          <div className="mt-2 flex items-center gap-1.5"><span className="inline-flex items-center gap-1.5 rounded-md bg-indigo-50 px-2 py-1 text-[13px] font-semibold text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400"><span className="h-1.5 w-1.5 rounded-full bg-indigo-500" />{user?.role || 'Administrateur'}</span></div>
                        </div>
                      </div>
                    </div>
                    <div className="p-1.5">
                      {/* ⭐ FIX: Text Light (text-slate-700) + Dark (text-slate-300) */}
                      ...
                      {/* ⭐ FIX: Logout Light (text-rose-600) + Dark (text-rose-400) */}
        ...
        <main className="flex-1 overflow-y-auto bg-slate-100 transition-colors duration-300 sm:p-6 dark:bg-[#0A0F1E]">{children}</main>
      </div>
    </div>
  );
}