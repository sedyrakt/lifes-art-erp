...
interface CommandesModalFormProps { isOpen: boolean; onClose: () => void; onSubmit: (e: React.FormEvent) => void; clients: Client[]; produits: Produit[]; selectedClientId: number | null; onClientChange: (id: number | null) => void; selectedProduits: SelectedProduct[]; onAddProduit: (id: number, quantite: number) => void; onUpdateQuantite: (id: number, quantite: number) => void; onRemoveProduit: (id: number) => void; onClearPanier: () => void; isDark?: boolean; }
const TVA_RATE = 0.2;
...
const SectionTitle: React.FC<{ icon: React.ReactNode; title: string; theme: typeof COLORS.light; borderClass: string }> = ({ icon, title, theme, borderClass }) => (
  <div className={`flex items-center gap-2.5 px-4 py-3 border-b ${borderClass}`} style={{ background: theme.surface }}>
    <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg" style={{ background: theme.primaryBg, color: theme.primary }}>{icon}</div>
    <span className="text-[14px] font-semibold" style={{ color: theme.text }}>{title}</span>
  </div>
);
...
const CommandesModalForm: React.FC<CommandesModalFormProps> = ({ isOpen, onClose, onSubmit, clients, produits, selectedClientId, onClientChange, selectedProduits, onAddProduit, onUpdateQuantite, onRemoveProduit, onClearPanier, isDark: propIsDark }) => {
  ...
  const totalHT = useMemo(() => selectedProduits.reduce((sum, item) => { const product = produits.find((p) => p.id === item.id); if (!product) return sum; return sum + product.prix_vente * item.quantite; }, 0), [produits, selectedProduits]);
  const totalTVA = totalHT * TVA_RATE;
  const totalTTC = totalHT + totalTVA;
  const totalQuantity = selectedProduits.reduce((sum, item) => sum + item.quantite, 0);
  ...
  const InfoRow: React.FC<{ label: string; value: React.ReactNode; icon?: React.ReactNode }> = ({ label, value, icon }) => (
    <div className="flex items-start justify-between gap-4">
      <div className="flex min-w-0 items-center gap-2">
        {icon && <span className="shrink-0" style={{ color: theme.subtle }}>{icon}</span>}
        <span className="truncate text-[14px] font-medium" style={{ color: theme.muted }}>{label}</span>
      </div>
  ...
  const modal = (
    <div className={`fixed inset-0 z-[999999] flex items-center justify-center overflow-y-auto p-3 transition-opacity duration-200 ${isVisible ? 'opacity-100' : 'opacity-0'}`} style={{ background: theme.overlay, backdropFilter: 'blur(5px)' }} role="dialog" aria-modal="true" aria-labelledby="commande-modal-title" onMouseDown={handleOverlayMouseDown}>
      <div className={`relative flex w-full max-w-[950px] max-h-[85vh] flex-col overflow-hidden rounded-2xl border shadow-[0_24px_80px_rgba(0,0,0,0.22)] transition-all duration-200 ${isVisible ? 'translate-y-0 scale-100' : 'translate-y-2 scale-[0.985]'} ${borderClass}`} style={{ background: theme.card }} onMouseDown={(event) => event.stopPropagation()}>
        ...
        <header className={`flex shrink-0 items-center justify-between gap-4 border-b px-5 py-4 sm:px-6 ${borderClass}`} style={{ background: theme.surface }}>
          <div className="flex min-w-0 items-center gap-3">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border" style={{ borderColor: theme.border, background: theme.surfaceSoft }}>
              <ShoppingCart size={20} strokeWidth={2} style={{ color: theme.primary }} />
            </div>
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <h2 id="commande-modal-title" className="truncate text-[16px] font-semibold tracking-tight" style={{ color: theme.text }}>Nouvelle commande</h2>
                <span className="hidden shrink-0 rounded-md border px-2 py-0.5 text-[12px] font-medium sm:inline-flex" style={{ color: theme.primary, background: theme.primaryBg, borderColor: theme.primaryBorder }}>Vente</span>
              </div>
              ...
              <aside className="flex flex-col gap-3">
                <div className={`overflow-hidden rounded-xl border ${borderClass}`} style={{ background: theme.surface }}>
                  ...
                  <div className="space-y-3 p-4">
                    <InfoRow label="Client" value={selectedClient?.nom || '—'} icon={<User size={13} />} />
                    <div className="h-px" style={{ background: theme.border }} />
                    <InfoRow label="Références" value={String(selectedProduits.length)} icon={<ShoppingCart size={13} />} />
                    <div className="h-px" style={{ background: theme.border }} />
                    <InfoRow label="Qté totale" value={String(totalQuantity)} icon={<Box size={13} />} />
                    <div className="h-px" style={{ background: theme.border }} />
                    <InfoRow label="Total HT" value={formatMoney(totalHT)} icon={<Clock size={13} />} />
                    <div className="h-px" style={{ background: theme.border }} />
                    <InfoRow label="TVA (20%)" value={formatMoney(totalTVA)} icon={<Clock size={13} />} />
                    <div className="h-px" style={{ background: theme.border }} />
                ...
                {(!selectedClientId || selectedProduits.length === 0) && (
                  <div className={`flex items-start gap-2 rounded-lg border px-3 py-2.5 ${borderClass}`} style={{ borderColor: isDark ? 'rgba(248,113,113,0.18)' : '#FECACA', background: theme.dangerBg }}>
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-0.5 text-[13px] leading-4" style={{ color: theme.muted }}>
                        ...
                        {selectedProduits.length === 0 && 'Ajoutez au moins un produit.'}
                      </p>
                    </div>
                  </div>
                )}
                ...
                <div className={`overflow-hidden rounded-xl border ${borderClass}`}>
                  <div className="p-4">
                    <div className="grid grid-cols-1 gap-3.5">
                      <div>
                        <label className="mb-1.5 block flex items-center gap-1.5 text-[12px] font-semibold uppercase tracking-wider" style={{ color: theme.muted }}>
                          <User size={13} />Client <span className="ml-1" style={{ color: theme.danger }}>*</span>
                        </label>
                  ...
                  <div className={`border-t ${borderClass}`}>
                    <CommandesProductSelector produits={produits} selectedProduits={selectedProduits} onAddProduit={onAddProduit} onUpdateQuantite={onUpdateQuantite} onRemoveProduit={onRemoveProduit} onClearPanier={onClearPanier} theme={theme} isDark={isDark} />
                  </div>
                </div>
              </div>
            </div>
        ...
        <footer className={`flex shrink-0 items-center justify-end gap-2 border-t px-5 py-3 sm:px-6 ${borderClass}`} style={{ background: theme.surfaceSoft }}>
          ...
          <button type="button" onClick={() => formRef.current?.requestSubmit()} disabled={!selectedClientId || selectedProduits.length === 0} className="flex h-9 items-center gap-2 rounded-lg px-4 text-[14px] font-semibold text-white shadow-sm transition-all hover:shadow-md active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50" style={{ background: theme.primary }} onMouseEnter={e => { if (selectedClientId && selectedProduits.length > 0) e.currentTarget.style.background = theme.primaryHover; }} onMouseLeave={e => { e.currentTarget.style.background = theme.primary; }}>
            <CheckCircle className="h-3.5 w-3.5" strokeWidth={2} /> Valider la commande
          </button>
        </footer>
      </div>
    </div>
