...
import { Search, X, List, Grid, ArrowUpDown, ChevronDown, Calendar } from 'lucide-react';
  ...
  return (<div className="w-full" role="search" aria-label="Barre de recherche et filtres des commandes">
    <div className="flex w-full flex-col gap-2.5 xl:flex-row xl:items-center">
      ...
      <div className="flex w-full flex-wrap items-center gap-2 xl:w-auto xl:shrink-0">
        ...
        {/* ⭐ FIX: Calendar Date Filter - MISY VALUE SY ONCHANGE */}
        <div className="relative shrink-0">
          <Calendar size={14} strokeWidth={2} className={`pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 ${isDark ? 'text-slate-500' : 'text-slate-400'}`} />
          <input
            type="date"
            value={filterDateFrom}
            onChange={(event) => onFilterDateFromChange(event.target.value)}
            className={`h-10 w-[140px] cursor-pointer appearance-none rounded-lg border ${border} ${surface} pl-9 pr-2 text-[14px] font-medium ${isDark ? 'text-slate-300' : 'text-slate-700'} outline-none shadow-sm transition-all duration-200 ${hoverBorder} focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/10`}
          />
          
          {/* ⭐ FIX: Bouton Reset Date */}
          {filterDateFrom && (
            <button
              type="button"
              onClick={() => {
              ...
              className="absolute right-2 top-1/2 flex h-6 w-6 -translate-y-1/2 items-center justify-center rounded-md bg-slate-100 text-slate-500 hover:bg-slate-200 hover:text-slate-700 dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-slate-700 dark:hover:text-slate-200"
