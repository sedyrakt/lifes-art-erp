...
interface CommandesProductSelectorProps { produits: Produit[]; selectedProduits: SelectedProduct[]; onAddProduit: (id: number, quantite: number) => void; onUpdateQuantite: (id: number, quantite: number) => void; onRemoveProduit: (id: number) => void; onClearPanier: () => void; theme: any; isDark: boolean; }
        ...
        <div className="absolute left-0 right-0 z-[80] mt-1 max-h-60 overflow-y-auto rounded-lg border py-1 shadow-xl" style={{ background: isDark ? '#0F172A' : '#FFFFFF', borderColor: theme.border }}>
          {filteredOptions.length === 0 ? (
            <div className="flex flex-col items-center justify-center px-4 py-8 text-center">
              ...
              <p className="text-[14px] font-medium text-slate-500 dark:text-slate-400">Aucun produit trouvé</p>
            </div>
          ) : (
...
const CommandesProductSelector: React.FC<CommandesProductSelectorProps> = ({ produits, selectedProduits, onAddProduit, onUpdateQuantite, onRemoveProduit, onClearPanier, theme, isDark }) => {
  const [selectedProductId, setSelectedProductId] = useState('');
  const [quantityValue, setQuantityValue] = useState('1');
  const [productImages, setProductImages] = useState<Record<number, string | null>>({});
  const [stockMessage, setStockMessage] = useState('');
  const produitsDisponibles = useMemo(() => produits.filter((product) => Number(product.quantite_stock) > 0), [produits]);
  const borderClass = isDark ? 'border-slate-700' : 'border-gray-300';

  // ⭐ FIX: Nampidirina ny selectedProduits ho dependency
  useEffect(() => {
    let cancelled = false;
    const loadImages = async () => {
      ...
      for (const product of selectedProducts) {
        if (cancelled || productImages[product.id]) continue;
        try {
  ...
  }, [selectedProduits, produits]); // ⭐ ADD: selectedProduits
  ...
  const handleAdd = () => {
    ...
    if (!product || product.quantite_stock <= 0) {
      setStockMessage('Ce produit est actuellement en rupture de stock.');
      return;
    }
    ...
    else onAddProduit(productId, quantity);
    setSelectedProductId('');
    setQuantityValue('1');
    setStockMessage('');
  };

  const totalPanier = useMemo(() => selectedProduits.reduce((total, item) => {
    const product = produits.find((p) => p.id === item.id);
    if (!product) return total;
    return total + product.prix_vente * item.quantite;
      ...
      <div className={`border-b ${borderClass}`}>
        <div className="flex items-center justify-between gap-3 bg-slate-50/70 px-5 py-3 dark:bg-[#111c30]">
          <div className="flex min-w-0 items-center gap-2.5">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 truncate text-[12px] text-slate-500 dark:text-slate-400">Recherchez un produit et indiquez la quantité.</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 gap-3 p-4 md:grid-cols-[minmax(0,1fr)_120px_110px]">
          <div className="flex flex-col gap-1.5">
            <label className="flex items-center gap-1.5 text-[12px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400"><Package size={13} className="text-slate-400" />Produit</label>
            <ProductSearchDropdown options={produitsDisponibles} value={selectedProductId} onChange={setSelectedProductId} placeholder="Rechercher un produit..." isDark={isDark} theme={theme} />
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="flex items-center gap-1.5 text-[12px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400"><Hash size={13} className="text-slate-400" />Quantité</label>
      ...
      <div className="bg-white dark:bg-[#0F172A]">
        ...
        {selectedProduits.length === 0 ? (
          <div className="flex flex-col items-center justify-center px-5 py-12 text-center">
            ...
            <p className="mt-1 text-[13px] text-slate-500 dark:text-slate-400">Ajoutez des produits pour commencer.</p>
          </div>
        ) : (
