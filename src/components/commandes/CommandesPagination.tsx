...
const CommandesPagination: React.FC<CommandesPaginationProps> = ({ 
  ...
  const pageNumbers = useMemo(() => {
    ...
    // Raha tianao hiseho ilay bouton 1 na dia tokana aza ny pejy dia tokony hiverina [1] ity
  ...
  return (
    <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4 px-2 w-full">
      <div className="text-xs font-bold uppercase tracking-wider" style={{ color: theme.muted }}>
        Total : <span style={{ color: theme.text }} className="font-black">{totalItems}</span> commande{totalItems > 1 ? 's' : ''}
      </div>
