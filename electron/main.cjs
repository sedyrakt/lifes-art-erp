...
let mainWindow = null, databaseInitialized = false, handlersRegistered = false, isQuitting = false;
...
function printDatabaseInfo() {
  try {
    ...
    console.log('📦 LIFE\'S ART - DATABASE PATH'); console.log('============================================================');
    console.log('📁 userData :', app.getPath('userData')); console.log('📁 DB       :', dbPath);
    console.log('📦 exists   :', fs.existsSync(dbPath));
    if (fs.existsSync(dbPath)) {
      try { const stat = fs.statSync(dbPath); console.log('📊 size     :', stat.size, 'bytes'); console.log('🕒 modified :', stat.mtime.toISOString()); } catch (statErr) { mainWarn('⚠️ Impossible lire stat DB:', statErr.message); }
    }
    console.log('============================================================'); console.log('');
...
async function initializeDatabase() {
  ...
  try {
    ...
    mainLog('============================================================'); mainLog('✅ DATABASE READY'); mainLog('📁 Path:', getDbPath()); mainLog('🟢 Open:', db.open); mainLog('============================================================');
    try {
      const debugInfo = getDatabaseDebugInfo();
      if (debugInfo?.success) { mainLog('📊 Produits total:', debugInfo.totalProduits); mainLog('🟢 Produits actifs:', debugInfo.produitsActifs); mainLog('📦 Produits archivés:', debugInfo.produitsArchives); }
    } catch (debugErr) { mainWarn('⚠️ Database debug ignoré:', debugErr.message); }
    return true;
  } catch (err) {
    databaseInitialized = false;
    mainError('============================================================'); mainError('❌ DATABASE INITIALIZATION FAILED'); mainError(err.message);
...
function shutdownDatabase() {
  try {
    ...
    mainLog('🔌 Fermeture de la database...'); closeDatabase(); databaseInitialized = false; mainLog('✅ Database fermée');
  } catch (err) { mainWarn('⚠️ Erreur fermeture DB:', err.message); databaseInitialized = false; }
}
  ...
  try {
    protocol.handle('local-image', async (request) => {
      try {
        ...
        return new Response(await fs.promises.readFile(requestedPath), { status: 200, headers: { 'Content-Type': getMimeType(requestedPath), 'Cache-Control': 'public, max-age=31536000, immutable' } });
      } catch (err) { mainError('❌ local-image:// error:', err.message); return new Response('Internal Server Error', { status: 500 }); }
    });
    mainLog('✅ local-image:// protocol enregistré');
  } catch (err) { mainError('❌ Impossible enregistrer local-image://:', err.message); }
}

function isAllowedExternalUrl(url) { try { const parsed = new URL(url); return parsed.protocol === 'https:' || parsed.protocol === 'http:'; } catch (_) { return false; } }
...
function createMainWindow() {
  if (mainWindow) { if (!mainWindow.isDestroyed()) { mainWindow.show(); mainWindow.focus(); } return mainWindow; }
  ...
  mainWindow.webContents.setWindowOpenHandler(({ url }) => { if (isAllowedExternalUrl(url)) shell.openExternal(url); else mainWarn('🚫 URL externe bloquée:', url); return { action: 'deny' }; });
  ...
  mainWindow.on('close', (event) => { if (!isQuitting) {} });
...
async function loadFrontend() {
  ...
  try {
    ...
    if (isDev && mainWindow && !mainWindow.isDestroyed()) {
      mainWarn('⚠️ Frontend mbola tsy loaded correctement');
      try { showMainWindow('load-error-fallback'); } catch (fallbackErr) { mainError('❌ Impossible afficher fallback:', fallbackErr.message); }
    }
  }
}
    ...
    if (typeof handler === 'function') { result = handler(ipcMain); }
      ...
      for (const key of Object.keys(handler)) {
        if (typeof handler[key] === 'function' && key.toLowerCase().includes('register')) {
          ...
          mainLog(`✅ IPC ${label} enregistré via dynamic key: ${key}`);
          break;
        }
      }
    }
    else if (handler && typeof handler.default === 'function') { result = handler.default(ipcMain); }
    else { mainWarn(`⚠️ Module IPC ${label} chargé mais aucune fonction d'enregistrement trouvée`); return false; }
    if (!result) { mainWarn(`⚠️ IPC ${label} function executed but returned false`); return false; }
    mainLog(`✅ IPC ${label} enregistré avec succès`);
    return true;
  } catch (err) { mainError(`❌ Erreur IPC ${label}:`, err.message); if (err.stack) mainError(err.stack); return false; }
}

function registerAllIPC() {
  if (handlersRegistered) { mainLog('ℹ️ IPC handlers déjà enregistrés'); return; }
  mainLog('============================================================'); mainLog('🔌 ENREGISTREMENT IPC HANDLERS'); mainLog('============================================================');
  const handlerModules = [
    ...
    ['REPORTS', './ipc/reports.cjs'], ['IMAGES', './ipc/images.cjs'],
    ['SETTINGS', './ipc/settings.cjs'],
    ['BACKUP', './ipc/backup.cjs'],
    ['DIALOG', './ipc/dialog.cjs']
  ];
  let successCount = 0;
  for (const [label, modulePath] of handlerModules) { if (registerHandlerModule(label, modulePath)) successCount++; }
  try {
    if (!ipcMain.listenerCount('db:getPath')) { ipcMain.handle('db:getPath', async () => { try { return { success: true, path: getDbPath() }; } catch (err) { return { success: false, error: err.message }; } }); }
    if (!ipcMain.listenerCount('db:debug')) { ipcMain.handle('db:debug', async () => { try { return getDatabaseDebugInfo(); } catch (err) { return { success: false, error: err.message }; } }); }
    mainLog('✅ IPC DB debug enregistré');
  } catch (err) { mainWarn('⚠️ IPC DB debug:', err.message); }
  try {
    if (!ipcMain.listenerCount('app:getInfo')) {
      ipcMain.handle('app:getInfo', async () => { return { success: true, name: app.getName(), version: app.getVersion(), isPackaged: app.isPackaged, isDev, userData: app.getPath('userData'), dbPath: getDbPath() }; });
    }
...
function setupSingleInstance() {
  ...
  if (!gotLock) { mainWarn('⚠️ Une autre instance de LIFE\'S ART est déjà ouverte.'); app.quit(); return false; }
  app.on('second-instance', (event, commandLine, workingDirectory) => { mainLog('ℹ️ Deuxième instance détectée'); if (mainWindow && !mainWindow.isDestroyed()) { if (mainWindow.isMinimized()) mainWindow.restore(); mainWindow.show(); mainWindow.focus(); } });
  return true;
}

app.whenReady().then(async () => {
  mainLog('============================================================'); mainLog('🚀 LIFE\'S ART ELECTRON START'); mainLog('============================================================');
  mainLog('📦 Electron:', process.versions.electron); mainLog('🟢 Node:', process.versions.node); mainLog('🟢 Chromium:', process.versions.chrome);
  mainLog('📦 Packaged:', app.isPackaged); mainLog('🛠️ Development:', isDev); mainLog('📁 userData:', app.getPath('userData'));
  try { Menu.setApplicationMenu(null); } catch (err) { mainWarn('⚠️ Impossible supprimer menu:', err.message); }
  configureSecurity();
  registerLocalImageProtocol();
  try { await initializeDatabase(); } catch (dbErr) { mainError('❌ Database initialization failed:', dbErr.message); app.quit(); return; }
  registerAllIPC();
  createMainWindow();
  app.on('activate', () => { if (BrowserWindow.getAllWindows().length === 0) createMainWindow(); });
  mainLog('============================================================'); mainLog('🟢 LIFE\'S ART READY'); mainLog('============================================================');
}).catch((err) => {
  mainError('============================================================'); mainError('❌ ELECTRON STARTUP FAILED'); mainError(err.message);
  if (err.stack) mainError(err.stack); mainError('============================================================'); app.quit();
});

app.on('before-quit', () => { if (isQuitting) return; isQuitting = true; mainLog('🛑 before-quit'); shutdownDatabase(); });
app.on('window-all-closed', () => { mainLog('🪟 Toutes les fenêtres sont fermées'); if (process.platform !== 'darwin') { if (!isQuitting) isQuitting = true; shutdownDatabase(); app.quit(); } });
app.on('will-quit', () => { mainLog('🛑 will-quit'); shutdownDatabase(); });
process.on('SIGINT', () => { mainLog('🛑 SIGINT reçu'); shutdownDatabase(); if (!isQuitting) { isQuitting = true; app.quit(); } });
process.on('SIGTERM', () => { mainLog('🛑 SIGTERM reçu'); shutdownDatabase(); if (!isQuitting) { isQuitting = true; app.quit(); } });
process.on('uncaughtException', (err) => { mainError('============================================================'); mainError('❌ UNCAUGHT EXCEPTION'); mainError(err.message); if (err.stack) mainError(err.stack); mainError('============================================================'); });
process.on('unhandledRejection', (reason) => { mainError('============================================================'); mainError('❌ UNHANDLED REJECTION'); if (reason instanceof Error) { mainError(reason.message); if (reason.stack) mainError(reason.stack); } else mainError(reason); mainError('============================================================'); });
if (isDev) { mainLog('🛠️ main.cjs chargé en mode development'); mainLog('📁 APP_ROOT:', APP_ROOT); mainLog('📁 PRELOAD:', PRELOAD_PATH); mainLog('📁 DIST:', DIST_PATH); }
