...
const bcrypt = require('bcryptjs'), jwt = require('jsonwebtoken'), speakeasy = require('speakeasy'), QRCode = require('qrcode');
...
// ⭐ FANITSARA FARANY: Tsy misy fanamarinana intsony - Constant mivantana
const JWT_SECRET = 'upA8lN2MfcFQBPge8AbGSRQDVGRwXuqFnfb3B0xh1+2l2oiHmOKdMhoQvzwfv5aqE2Io4a+EFPEWPliSVyiXBQ==';
const JWT_EXPIRES_IN = '24h', SESSION_EXPIRY_HOURS = 24, TWO_FACTOR_ENABLED = true, TWO_FACTOR_WINDOW = 1, TWO_FACTOR_APP_NAME = 'FITAIA', BCRYPT_SALT_ROUNDS = 12;
...
const cleanupExpiredSessions = () => {
  try {
    ...
    if (!statement) { error('⚠️ [auth] stmtDeleteExpiredSessions indisponible'); return; }
...
function registerAuthHandlers(ipcMain) {
  ...
  try {
    ...
    const channels = [
      'auth:login', 'auth:logout', 'auth:verify-token', 'auth:hash-password',
      'auth:verify-password', 'auth:change-password', 'auth:generate-2fa',
      'auth:verify-2fa', 'auth:disable-2fa', 'auth:verify-2fa-login',
      'auth:reset-password', 'auth:rehash-password', 'auth:get-sessions',
      'auth:revoke-session', 'auth:revoke-all-sessions',
      'auth:check-user-exists', 'auth:get-user'
    ];
    ...
    ipcMain.handle('auth:login', withAuthDb(async (event, email, password, ip, userAgent) => {
      try {
        ...
        if (!user) {
          ...
          await logSecurityEvent(normalizedEmail, 'login_failed', safeIp, safeUserAgent, 0, 'Utilisateur non trouvé');
          throw new Error('Email ou mot de passe incorrect');
        }

        let isValid = false;
        try { isValid = await bcrypt.compare(cleanPassword, user.password); }
        catch (err) { log(`❌ Erreur bcrypt.compare: ${err.message}`); }

        if (!isValid) {
          log('❌ [auth:login] Mot de passe incorrect');
          await logSecurityEvent(normalizedEmail, 'login_failed', safeIp, safeUserAgent, 0, 'Mot de passe incorrect');
          throw new Error('Email ou mot de passe incorrect');
        }

        if (TWO_FACTOR_ENABLED && user.twoFactorEnabled === 1) {
          log('🔐 [auth:login] 2FA activé');
          await logSecurityEvent(normalizedEmail, '2fa_required', safeIp, safeUserAgent, 1);
          return { success: true, need2FA: true, userId: user.id, message: 'Code 2FA requis' };
        }
        ...
        return {
          ...
          user: {
            ...
            role: user.role, companyName: user.companyName, phone: user.phone, image: user.image,
            created_at: user.created_at, twoFactorEnabled: user.twoFactorEnabled === 1
          }
        };
      } catch (err) {
    ...
    ipcMain.handle('auth:logout', withAuthDb(async (event, token) => {
      try {
        if (!statementsModule.stmtDeleteSession) throw new Error('Service de base de données non disponible');
    ...
    ipcMain.handle('auth:hash-password', withAuthDb(async (event, password) => {
      try {
        const cleanPassword = password ? password.trim() : '';
        if (!cleanPassword) throw new Error('Le mot de passe est requis');

        const salt = await bcrypt.genSalt(BCRYPT_SALT_ROUNDS);
        const hash = await bcrypt.hash(cleanPassword, salt);

        if (!hash || hash.length !== 60) throw new Error('Erreur lors du hashage');
    ...
    ipcMain.handle('auth:verify-password', withAuthDb(async (event, password, hashedPassword) => {
      try {
        if (!hashedPassword || typeof hashedPassword !== 'string' || hashedPassword.length !== 60) return false;
    ...
    ipcMain.handle('auth:change-password', withAuthDb(async (event, data) => {
      try {
        if (!statementsModule.stmtGetUserByIdWithStatus || !statementsModule.stmtUpdateUserPassword || !statementsModule.stmtDeleteSessionsByUser) {
          throw new Error('Service de base de données non disponible');
        }

        const userId = Number(data?.userId), oldPassword = data?.oldPassword, newPassword = data?.newPassword;
        ...
        if (!hashedPassword) throw new Error('Erreur lors du hashage');
    ...
    ipcMain.handle('auth:verify-2fa', withAuthDb(async (event, userId, secret, token) => {
      try {
        const verified = speakeasy.totp.verify({ secret, encoding: 'base32', token, window: TWO_FACTOR_WINDOW });

        if (verified) {
          if (!statementsModule.stmtUpdateUser2FA) throw new Error('Service de base de données non disponible');
    ...
    ipcMain.handle('auth:disable-2fa', withAuthDb(async (event, userId) => {
      try {
        if (!statementsModule.stmtUpdateUser2FADisable) throw new Error('Service de base de données non disponible');
    ...
    ipcMain.handle('auth:verify-2fa-login', withAuthDb(async (event, userId, token) => {
      try {
        ...
        const userData = statementsModule.stmtGetUserByIdWithStatus.get(userId);
        ...
        return {
          ...
          token: jwtToken,
          user: {
            id: userData.id, email: userData.email, firstName: userData.firstName, lastName: userData.lastName,
            role: userData.role, companyName: userData.companyName, phone: userData.phone, image: userData.image,
            created_at: userData.created_at, twoFactorEnabled: userData.twoFactorEnabled === 1
          }
        };
      } catch (err) {
    ...
    ipcMain.handle('auth:reset-password', withAuthDb(async (event, email, newPassword) => {
      try {
        if (!statementsModule.stmtGetUserByEmail || !statementsModule.stmtUpdateUserPassword || !statementsModule.stmtDeleteSessionsByUser) {
        ...
        await logSecurityEvent(normalizedEmail, 'password_reset', '127.0.0.1', 'Electron', 1);
    ...
    ipcMain.handle('auth:rehash-password', withAuthDb(async (event, email, newPassword) => {
      try {
        ...
        error('❌ [auth:rehash] Erreur:', err.message);
        return { success: false, error: err.message };
      }
    }));
    ...
    ipcMain.handle('auth:get-sessions', withAuthDb(async (event, userId) => {
      try {
        if (!statementsModule.stmtGetSessionsByUser) throw new Error('Service de base de données non disponible');
        return statementsModule.stmtGetSessionsByUser.all(userId);
      } catch (err) {
    ...
    ipcMain.handle('auth:revoke-all-sessions', withAuthDb(async (event, userId) => {
      try {
        if (!statementsModule.stmtDeleteSessionsByUser) throw new Error('Service de base de données non disponible');
        statementsModule.stmtDeleteSessionsByUser.run(userId);
        return { success: true };
      } catch (err) {
    ...
    ipcMain.handle('auth:check-user-exists', withAuthDb(async (event, email) => {
      try {
        if (!statementsModule.stmtGetUserByEmail) return null;
        const normalizedEmail = normalizeEmail(email);
        return statementsModule.stmtGetUserByEmail.get(normalizedEmail) || null;
      } catch (err) {
    ...
    ipcMain.handle('auth:get-user', withAuthDb(async (event, userId) => {
      try {
        if (!statementsModule.stmtGetUserById) return null;
        return statementsModule.stmtGetUserById.get(userId);
      } catch (err) {
    ...
    for (const channel of channels) log(`   - ${channel}: enregistré`);
