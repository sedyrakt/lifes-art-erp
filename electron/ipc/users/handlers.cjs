...
async function hashPassword(password) { if (!password) throw new Error('Mot de passe requis'); const cleanPassword = password.trim(); if (cleanPassword.length < 8) throw new Error('Le mot de passe doit contenir au moins 8 caractères'); const salt = await bcrypt.genSalt(BCRYPT_SALT_ROUNDS); return await bcrypt.hash(cleanPassword, salt); }
function logAudit(action, userId, userEmail, details = '') { try { const db = getDb(); const stmt = db.prepare(`INSERT INTO audit_logs (action, entity, entity_id, entity_name, user_id, details, created_at) VALUES (?, 'utilisateur', ?, ?, ?, ?, datetime('now'))`); stmt.run(action, userId, userEmail, userId, details); } catch (_) { /* ignore */ } }
...
function safeInsert(params) { if (!statementsModule.statementsReady || !statementsModule.stmtInsert) { console.error('❌ [users] Statements not ready ou stmtInsert null!'); return { lastInsertRowid: null, changes: 0, error: 'Erreur interne: base de données non initialisée' }; } try { const result = statementsModule.stmtInsert.run(...params); return { lastInsertRowid: result.lastInsertRowid, changes: result.changes }; } catch (err) { console.error('❌ [users] Erreur INSERT:', err.message); return { lastInsertRowid: null, changes: 0, error: err.message }; } }
function safeUpdate(params) { if (!statementsModule.statementsReady || !statementsModule.stmtUpdate) { console.error('❌ [users] Statements not ready ou stmtUpdate null!'); return { changes: 0, error: 'Erreur interne: base de données non initialisée' }; } try { const result = statementsModule.stmtUpdate.run(...params); return { changes: result.changes }; } catch (err) { console.error('❌ [users] Erreur UPDATE:', err.message); return { changes: 0, error: err.message }; } }
...
function ensureStatements() { const result = statementsModule.prepareStatements(); if (!result) error('❌ [users] prepareStatements a échoué'); return result; }

function registerUsersHandlers(ipcMain) {
  log('👥 =========================================='); log('👥 [users.handlers] ENREGISTREMENT HANDLERS USERS'); log('👥 ==========================================');
  ...
  ipcMain.handle('users:create', async (event, data) => {
    try { log('📝 [users:create] Données reçues'); const { email, password, firstName, lastName, role, companyName, phone } = data;
      ...
      const normalizedEmail = normalizeEmail(email); const cleanPassword = pwdValidation.trimmed || password.trim();
      if (!ensureStatements()) return { success: false, error: 'Erreur interne: base de données non initialisée' };
      const existing = safeGetByEmailForCheck(normalizedEmail); if (existing) return { success: false, error: 'Cet email est déjà utilisé' };
      const hashedPassword = await hashPassword(cleanPassword); if (!hashedPassword || hashedPassword.length !== 60) return { success: false, error: 'Erreur lors du hashage' };
      ...
      const userId = result.lastInsertRowid; const db = getDb(); const checkStmt = db.prepare('SELECT id, email FROM utilisateurs WHERE id = ?'); const newUser = checkStmt.get(userId); if (!newUser) return { success: false, error: 'Erreur lors de la création du compte' };
      logAudit('user_created', userId, normalizedEmail); try { await logSecurityEvent(normalizedEmail, 'user_created', '127.0.0.1', 'Electron', 1, `ID: ${userId}`); } catch (_) {}
      log(`✅ Utilisateur créé: ${normalizedEmail} (ID: ${userId})`); return { success: true, data: { id: userId } };
  ...
  ipcMain.handle('users:update', async (event, id, data) => {
    try {
      ...
      const userId = parseInt(id); const existing = safeGetByIdForUpdate(userId); if (!existing) return { success: false, error: 'Utilisateur non trouvé' };
      ...
      if (password !== undefined && password !== null && String(password).trim() !== '') { const pwdValidation = validatePassword(String(password)); if (!pwdValidation.valid) return { success: false, error: pwdValidation.message }; const cleanPassword = pwdValidation.trimmed; finalPassword = await hashPassword(cleanPassword); if (!finalPassword || finalPassword.length !== 60) return { success: false, error: 'Erreur hashage' }; }
      ...
      if (result.error) return { success: false, error: result.error }; if (result.changes === 0) return { success: false, error: 'Aucune modification effectuée' };
      if (status && status !== 'actif') safeDeleteSessions(userId);
      logAudit('user_updated', userId, existing.email); try { await logSecurityEvent(existing.email, 'user_updated', '127.0.0.1', 'Electron', 1, `ID: ${userId}`); } catch (_) {}
      log(`✅ Utilisateur mis à jour: ${newEmail} (ID: ${userId})`); return { success: true };
    } catch (err) { error('❌ [users:update] Erreur:', err.message); return { success: false, error: err.message }; }
  });
  ipcMain.handle('users:delete', async (event, id) => { try { ensureStatements(); if (!id || isNaN(id) || parseInt(id) <= 0) return { success: false, error: 'ID invalide' }; const userId = parseInt(id); const existing = safeGetById(userId); if (!existing) return { success: false, error: 'Utilisateur non trouvé' }; safeDeleteSessions(userId); safeDeleteUser(userId); logAudit('user_deleted', userId, existing.email); try { await logSecurityEvent(existing.email, 'user_deleted', '127.0.0.1', 'Electron', 1, `ID: ${userId}`); } catch (_) {} return { success: true }; } catch (err) { error('❌ [users:delete] Erreur:', err.message); return { success: false, error: err.message }; } });
  ipcMain.handle('users:stats', () => { try { ensureStatements(); const stats = safeGetStats(); return { success: true, data: { total: stats?.total || 0, active: stats?.active || 0, admins: stats?.admins || 0 } }; } catch (err) { error('❌ [users:stats] Erreur:', err.message); return { success: false, error: err.message }; } });

  const registeredEvents = ipcMain.eventNames();
  log('📋 [users.handlers] Vérification finale:'); for (const ch of channels) log(`   - ${ch}: ${registeredEvents.includes(ch) ? '✅' : '❌'}`);
  log('👥 ==========================================');
  if (statementsModule.statementsReady) log('✅ Users handlers enregistrés (synchronous)'); else log('⚠️ Users handlers enregistrés MAIS la table "utilisateurs" est introuvable.');
  log('👥 ==========================================');
