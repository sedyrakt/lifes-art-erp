...
function registerEmployesHandlers(ipcMain) {
  ...
  const channels = ['employes:get-all','employes:get-by-id','employes:create','employes:update','employes:delete','employes:get-by-departement','employes:get-by-status','employes:get-stats','employes:search','employes:bulk-delete','employes:update-status','employes:bulk-update-status','employes:get-paiement-counts-batch'];
  ...
  ipcMain.handle('employes:get-all', async (event, options = {}) => {
    try {
      ...
      total = Number(countResult?.total || 0); const limit = options.limit || 50; totalPages = Math.ceil(total / limit);
