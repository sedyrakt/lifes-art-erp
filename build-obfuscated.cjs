...
// ⭐ 8. FIX: ESRINA NY private.pem SY fingerprint.txt amin'ny production
...
for (const filePath of sensitiveFiles) {
  if (fs.existsSync(filePath)) {
    try {
      ...
      console.log(`   ✅ ${path.basename(filePath)} esorina (production)`);
    } catch (err) {
      console.error(`   ❌ Erreur suppression ${path.basename(filePath)}:`, err.message);
    }
  } else {
    console.log(`   ⚠️ ${path.basename(filePath)} tsy hita, skip`);
  }
}
...
console.log('\n🔍 Vérification finale...');
const finalCheck = [
  { path: path.join(distDir, 'keys/public.pem'), required: true, name: 'public.pem' },
  { path: path.join(distDir, 'keys/private.pem'), required: false, name: 'private.pem' },
  { path: path.join(distDir, 'keys/fingerprint.txt'), required: false, name: 'fingerprint.txt' },
];

for (const check of finalCheck) {
  const exists = fs.existsSync(check.path);
  if (exists && check.required) {
    console.log(`   ✅ ${check.name} présent`);
  } else if (exists && !check.required) {
    console.log(`   ⚠️ ${check.name} présent (fa tsy ilaina)`);
  } else if (!exists && check.required) {
    console.log(`   ❌ ${check.name} manquant!`);
  } else {
    console.log(`   ✅ ${check.name} esorina (production)`);
  }
}
